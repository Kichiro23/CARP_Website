...
export default function Widget() {
  ...
  return (
    <div className="flex h-[200px] w-[320px] flex-col justify-between rounded-2xl border p-4" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--tile-border)' }}>
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>H: {weather.daily[0]?.maxTemp}° L: {weather.daily[0]?.minTemp}°</p>
        </div>
        <div className="text-right">
          ...
          <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>weathercarp.com</p>
        </div>
      </div>
    </div>
  );
